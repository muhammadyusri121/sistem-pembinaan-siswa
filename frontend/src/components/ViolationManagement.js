...
const ViolationManagement = () => {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'processed':
        return <span className="badge badge-info flex items-center gap-1">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            {violation.nis_siswa} • {student?.id_kelas}
                          </p>
                        </div>
                      </div>
                    </td>
                    <td>
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          {violationType && getViolationCategoryBadge(violationType.kategori)}
                    ...
                    <td>
                      <div className="space-y-1">
                        <div className="flex items-center gap-1 text-sm text-gray-600">
                          <Clock className="w-3 h-3" />
                          {new Date(violation.waktu_kejadian).toLocaleString('id-ID')}
                        </div>
                        <div className="flex items-center gap-1 text-sm text-gray-600">
        ...
        <div className="modal-overlay" onClick={() => setShowDetailModal(false)}>
          <div className="modal-content max-w-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-gray-900">Detail Pelanggaran</h2>
            ...
            <div className="space-y-6">
              ...
              <div className="p-4 bg-red-50 rounded-lg">
                <h3 className="font-semibold text-gray-900 mb-3">Detail Pelanggaran</h3>
                {(() => {
                  const violationType = getViolationTypeInfo(selectedViolation.jenis_pelanggaran_id);
                  return (
                    <div className="space-y-3 text-sm">
                      <div>
                        <p className="text-gray-600">Jenis Pelanggaran:</p>
                        <div className="flex items-center gap-2 mt-1">
                          <p className="font-medium">{violationType?.nama_pelanggaran}</p>
                          {violationType && getViolationCategoryBadge(violationType.kategori)}
                          <span className="text-xs text-gray-500">({violationType?.poin} poin)</span>
                        </div>
                        {violationType?.deskripsi && (
                          <p className="text-gray-600 mt-1">{violationType.deskripsi}</p>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="font-medium">
                            {new Date(selectedViolation.waktu_kejadian).toLocaleString('id-ID')}
                          </p>
                        </div>
                        <div>
                          <p className="text-gray-600">Tempat:</p>
                          <p className="font-medium">{selectedViolation.tempat}</p>
                        </div>
                      </div>
                      
                      <div>
                        <p className="text-gray-600">Detail Kejadian:</p>
                        <p className="font-medium mt-1">{selectedViolation.detail_kejadian}</p>
                      </div>
              ...
              <div className="p-4 bg-blue-50 rounded-lg">
                ...
                {(() => {
                  const reporter = getReporterInfo(selectedViolation.pelapor_id);
                  return reporter ? (
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <p className="text-gray-600">Nama:</p>
                      ...
                      <div>
                        ...
                        <p className="font-medium capitalize">{reporter.role.replace('_', ' ')}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {new Date(selectedViolation.created_at).toLocaleDateString('id-ID')}
                        </p>
                      </div>
                    </div>
                  ) : (
                    <p className="text-gray-500">Data pelapor tidak ditemukan</p>
                  );
                })()}
              </div>
              ...
              {selectedViolation.catatan_pembinaan && (
                <div className="p-4 bg-green-50 rounded-lg">
                  <h3 className="font-semibold text-gray-900 mb-3">Catatan Pembinaan</h3>
                  <p className="text-sm">{selectedViolation.catatan_pembinaan}</p>
                </div>
              )}

              {selectedViolation.tindak_lanjut && (
                <div className="p-4 bg-yellow-50 rounded-lg">
                  <h3 className="font-semibold text-gray-900 mb-3">Tindak Lanjut</h3>
